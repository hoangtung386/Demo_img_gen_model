...
# ===== PATCH (xem VENDOR.md) =============================================
logger = logging.getLogger("hidream-o1")

# PATCH 1 — flash-attn không được cài mặc định trong image runtime. Nguyên
# bản hard-code True ở forward_once() nên inference nổ ngay forward đầu tiên
# với ImportError. Bật lại bằng IMG_USE_FLASH_ATTN=true khi image có wheel
# flash-attn khớp torch + CUDA.
_USE_FLASH_ATTN = os.environ.get("IMG_USE_FLASH_ATTN", "").strip().lower() == "true"

# PATCH 4 — đường attention mặc định. "auto" = SDPA hai lượt (nhanh, không
# cần wheel); "mask" = đường 4D-mask gốc của upstream. Xem VENDOR.md.
_ATTN_MODE_DEFAULT = os.environ.get("IMG_ATTENTION_MODE", "auto").strip().lower() or "auto"


def _flash_attn_unavailable() -> bool:
    """PATCH 4 — flash-attn có import được không.

    Bản gốc chỉ assert BÊN TRONG forward, tức là lỗi nổ ra sau khi đã nạp
    xong 10GB weight và dựng xong toàn bộ sample. Kiểm tra trước để rơi về
    SDPA thay vì giết request.
    """
    from .qwen3_vl_transformers import _flash_attn_func

    return _flash_attn_func is None


def _as_pil(item):
    """PATCH 2 — nhận PIL.Image ngoài đường dẫn file.

    Nguyên bản chỉ gọi Image.open(path). Tầng Gradio của dự án này đưa vào
    PIL.Image; không có hàm này thì mọi request phải ghi ảnh ra đĩa rồi đọc
    lại chỉ để thoả chữ ký hàm.
    """
...
def generate_image(
    ...
    if keep_original_aspect and ref_image_paths and len(ref_image_paths) == 1:
        pil_orig = _as_pil(ref_image_paths[0])  # PATCH 2
        # PATCH 5 — upstream ghim trần 2048. Ở chế độ không-snap, lấy trần
        # theo kích thước người dùng yêu cầu: đây là knob DUY NHẤT hạ được
        # chi phí của đường editing, vì nó quyết định số token của ảnh ra.
        ref_max_size = 2048 if snap_resolution else max(width, height, PATCH_SIZE)
        preresized_ref_pil = resize_pilimage(pil_orig, ref_max_size, PATCH_SIZE)
        width, height = preresized_ref_pil.size
        logger.info(  # PATCH 3
            "keep_original_aspect: kích thước đích %dx%d lấy từ ảnh tham chiếu",
            width,
            height,
        )
    else:
        if keep_original_aspect:
            logger.warning(  # PATCH 3
                "keep_original_aspect cần đúng 1 ảnh tham chiếu; quay về "
                "snap độ phân giải mặc định."
            )
        if snap_resolution:
            w, h = find_closest_resolution(width, height)
        else:
            # PATCH 5 — bỏ snap: dùng đúng kích thước yêu cầu, chỉ làm tròn
            # xuống bội số PATCH_SIZE. RẺ HƠN NHIỀU (chi phí ~ số token =
            # w*h/1024, còn attention thì ~ bình phương số đó) nhưng NGOÀI
            # phân bố huấn luyện: model chỉ được train ở 11 độ phân giải
            # trong PREDEFINED_RESOLUTIONS. Dùng cho bản nháp/preview.
            w = max(PATCH_SIZE, (width // PATCH_SIZE) * PATCH_SIZE)
            h = max(PATCH_SIZE, (height // PATCH_SIZE) * PATCH_SIZE)
        if w != width or h != height:
            logger.warning(  # PATCH 3
                "%s độ phân giải %dx%d -> %dx%d",
                "Snap" if snap_resolution else "Căn",
                width, height, w, h,
            )
        width, height = w, h
    ...
    if not ref_image_paths:
        ...
        if preresized_ref_pil is not None:
            ...
            ref_pils = [_as_pil(p) for p in ref_image_paths]  # PATCH 2
        ...
        if layout_bboxes is not None and len(layout_bboxes) > 0 and preresized_ref_pil is None:
            try:
                ...
                logger.warning(  # PATCH 3
                    "layout_bboxes không hợp lệ: %s (%s)", layout_bboxes, e
                )
    ...
    # ===== PATCH 6 (xem VENDOR.md) =======================================
    # CFG chạy HAI forward pass mỗi bước — nó là 50% tổng chi phí. "Guidance
    # interval" (Kynkäänniemi et al. 2024, arXiv:2404.07724) chỉ ra rằng
    # guidance gần như vô dụng ở hai đầu quỹ đạo: ở bước đầu ảnh còn là
    # nhiễu, ở bước cuối chỉ còn tinh chỉnh tần số cao. Bỏ nhánh uncond
    # ngoài khoảng [lo, hi] cắt thẳng số forward pass.
    #
    # cfg_interval=(0.0, 1.0) → giữ nguyên hành vi gốc, không bỏ bước nào.
    cfg_lo, cfg_hi = (float(cfg_interval[0]), float(cfg_interval[1])) if cfg_interval else (0.0, 1.0)
    _n_steps = len(sched.timesteps)
    _cfg_on = [
        cfg_lo <= (i / max(1, _n_steps - 1)) <= cfg_hi for i in range(_n_steps)
    ]
    if guidance_scale > 1.0 and not all(_cfg_on):
        logger.info(  # PATCH 3
            "CFG interval [%.2f, %.2f]: %d/%d bước chạy uncond (tiết kiệm %d forward pass)",
            cfg_lo, cfg_hi, sum(_cfg_on), _n_steps, _n_steps - sum(_cfg_on),
        )
    # ===== HẾT PATCH 6 ====================================================

    # PATCH 4 — giải quyết đường attention MỘT LẦN, ngoài vòng lặp.
    _attn_mode = (attn_mode or _ATTN_MODE_DEFAULT).strip().lower()
    _use_flash = _USE_FLASH_ATTN or _attn_mode == "flash"
    if _use_flash and _flash_attn_unavailable():
        logger.warning(  # PATCH 3
            "attention_mode='flash' nhưng không import được flash_attn; "
            "quay về SDPA hai lượt."
        )
        _use_flash, _attn_mode = False, "sdpa"
    ...
    def forward_once(sample, z_in, t_pixeldit, precomputed_image_embeds=None, precomputed_deepstack_image_embeds=None):
        with torch.autocast(device.type, dtype=dtype, cache_enabled=False):
            kwargs = {
                ...
                "use_flash_attn": _use_flash,  # PATCH 1
                "attn_mode": _attn_mode,  # PATCH 4
    ...
    for step_idx, step_t in enumerate(tqdm.tqdm(sched.timesteps, desc="Generating")):
        ...
        if ref_patches is None:
            ...
            if len(samples) > 1 and _cfg_on[step_idx]:  # PATCH 6
            ...
            x_vis_list = []
            # PATCH 6 — bước ngoài khoảng CFG chỉ chạy nhánh cond.
            active_samples = samples if _cfg_on[step_idx] else samples[:1]
