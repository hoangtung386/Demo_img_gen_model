...
if flash_attn_version == "2":
    ...
    try:
        ...
        try:
            ...
            _flash_attn_func = None
# ===== PATCH 4 (xem VENDOR.md) ===========================================
# Đường attention nhanh KHÔNG cần wheel flash-attn.
#
# Đường "standard" của upstream dựng một mask 4D dày đặc
# [batch, 1, seq, seq] bằng float rồi đưa vào SDPA. Với 2048x2048 seq ~4150,
# mask đó là ~34MB, và — nặng hơn nhiều — một attn_mask tường minh CẤM
# PyTorch dùng backend flash: SDPA rơi về mem-efficient, nơi mask phải được
# expand ra [batch, heads, seq, seq] (~1.1GB) và copy contiguous LẠI TỪ ĐẦU
# ở MỖI layer, MỖI bước. 36 layer x 2 forward x 50 bước = 3600 lần.
#
# Nhưng mask đó có cấu trúc: token AR (text) là một TIỀN TỐ liên tục, causal
# với nhau; mọi token gen (tms + ảnh + ref) nằm sau và nhìn thấy TẤT CẢ.
# Vậy nó tách được thành hai lời gọi SDPA KHÔNG mask:
#   * hàng AR : SDPA(q[:n_ar], k[:n_ar], v[:n_ar], is_causal=True)
#   * hàng gen: SDPA(q[n_ar:], k, v, is_causal=False)
# Kết quả toán học GIỐNG HỆT mask cũ, nhưng cả hai lời gọi đều vào được
# backend flash của PyTorch. Đây là cùng chiến lược hai lượt mà
# `_run_decoder_flash` dùng, chỉ khác là không cần cài flash-attn.
_SDPA_GQA_PROBED = None


def _sdpa_supports_gqa() -> bool:
    """SDPA có nhận enable_gqa= không (torch >= 2.5, tuỳ backend).

    Có thì khỏi phải repeat_kv: model này là GQA 32 head / 8 kv-head, nên
    repeat_kv nhân bản K,V lên 4 lần trong VRAM ở mỗi layer, mỗi bước.

    Phải THỬ GỌI chứ không soi được chữ ký: scaled_dot_product_attention là
    builtin C, ``inspect.signature`` ném ValueError trên nó. Đo một lần rồi
    nhớ kết quả.
    """
    global _SDPA_GQA_PROBED
    if _SDPA_GQA_PROBED is None:
        q = torch.zeros(1, 2, 1, 8)
        kv = torch.zeros(1, 1, 1, 8)
        try:
            F.scaled_dot_product_attention(q, kv, kv, enable_gqa=True)
            _SDPA_GQA_PROBED = True
        except (TypeError, RuntimeError):
            _SDPA_GQA_PROBED = False
    return _SDPA_GQA_PROBED


def _block_prefix_sdpa(query, key, value, n_ar: int):
    """Attention cho mask "AR-prefix causal + phần còn lại bidirectional".

    ``query``/``key``/``value``: [B, H, S, D] (K/V có thể ít head hơn Q).
    ``n_ar``: số token AR ở đầu chuỗi.
    """
    if _sdpa_supports_gqa():
        kv_kwargs = {"enable_gqa": key.shape[1] != query.shape[1]}
    else:
        n_rep = query.shape[1] // key.shape[1]
        key, value = repeat_kv(key, n_rep), repeat_kv(value, n_rep)
        kv_kwargs = {}

    out_gen = F.scaled_dot_product_attention(
        query[:, :, n_ar:], key, value, is_causal=False, **kv_kwargs
    )
    if n_ar <= 0:
        return out_gen
    ...
    return torch.cat([out_ar, out_gen], dim=2)
# ===== HẾT PATCH 4 =======================================================
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    # ===== PATCH 4 (xem VENDOR.md) =======================================
    def _run_decoder_sdpa(self, inputs_embeds, position_ids, token_types, n_ar,
                          visual_pos_masks=None, deepstack_visual_embeds=None,
                          return_mid_results_layers=None):
        """Bản song sinh của ``_run_decoder_flash`` chạy trên SDPA của PyTorch.

        Cùng cách tách hai lượt (causal trên tiền tố AR, bidirectional trên
        toàn chuỗi) nhưng dùng ``F.scaled_dot_product_attention`` nên KHÔNG
        cần wheel flash-attn — chạy được trên mọi image runtime.

        Điều kiện áp dụng: token AR phải là một TIỀN TỐ liên tục. Đúng với
        cả hai đường của ``pipeline.generate_image`` (t2i và editing): mọi
        token gen — tms, ảnh đích, ref patch — đều nằm sau phần text. Gọi
        hàm này với layout khác là sai kết quả, nên ``_forward_generation``
        kiểm tra trước và rơi về đường mask nếu không thoả.
        """
        text_model = self.language_model
        ...
        # ``n_ar`` do ``_ar_tokens_are_a_prefix`` tính sẵn và truyền vào: nó
        # cần một lần đồng bộ GPU→CPU (``.item()``), và một forward pass thì
        # không đáng trả hai lần cho cùng một con số.
        hidden_states = inputs_embeds
        mid_results = [] if return_mid_results_layers else None

        def _sdpa_layer_forward(hidden_states, decoder_layer, cos, sin, n_ar):
            original_attn_forward = decoder_layer.self_attn.forward

            def _custom_sdpa_attn(hidden_states, position_embeddings, attention_mask=None, **kwargs):
                attn = decoder_layer.self_attn
                input_shape = hidden_states.shape[:-1]
                ...
                out = _block_prefix_sdpa(q, k, v, n_ar)
                out = out.transpose(1, 2).reshape(*input_shape, -1).contiguous()
                return attn.o_proj(out), None

            _saved_gc = decoder_layer.gradient_checkpointing
            decoder_layer.gradient_checkpointing = False
            decoder_layer.self_attn.forward = _custom_sdpa_attn
            try:
                hidden_states = decoder_layer(
                    hidden_states,
                    position_embeddings=(cos, sin),
        ...
        for layer_idx, decoder_layer in enumerate(text_model.layers):
            if use_gc:
                hidden_states = torch.utils.checkpoint.checkpoint(
                    _sdpa_layer_forward,
                    hidden_states, decoder_layer, cos, sin, n_ar,
                    use_reentrant=False,
                )
            else:
                hidden_states = _sdpa_layer_forward(
                    hidden_states, decoder_layer, cos, sin, n_ar,
                )
    ...
    @staticmethod
    def _ar_tokens_are_a_prefix(token_types) -> tuple[bool, int]:
        """(áp dụng được?, số token AR) cho đường SDPA hai lượt.

        Trả False khi token AR không liên tục ở đầu chuỗi, hoặc khi các mẫu
        trong batch có layout khác nhau — hai trường hợp mà phép tách hai
        lượt không còn tương đương với mask 4D.
        """
        is_gen = token_types.bool()
        if is_gen.shape[0] > 1 and not bool((is_gen == is_gen[0]).all()):
            return False, 0
        row = is_gen[0]
        n_ar = int((~row).sum().item())
        # Mọi token AR phải nằm trước mọi token gen.
        if n_ar and bool(row[:n_ar].any()):
            return False, 0
        return True, n_ar
    # ===== HẾT PATCH 4 ===================================================
    ...
    def _forward_generation(self, input_ids, position_ids, vinputs, timestep, token_types,
        ...
        # PATCH 4 — chọn đường attention. "auto" ưu tiên SDPA hai lượt (không
        # cần wheel nào, không dựng mask dày đặc); "mask" là đường gốc của
        # upstream, giữ lại làm mốc so sánh khi nghi ngờ numerics.
        use_sdpa_fast, n_ar = False, 0
        if not use_flash_attn and attn_mode in (None, "auto", "sdpa"):
            use_sdpa_fast, n_ar = self._ar_tokens_are_a_prefix(token_types)
            if not use_sdpa_fast and attn_mode == "sdpa":
                logging.getLogger("hidream-o1").warning(
                    "attn_mode='sdpa' nhưng token AR không phải tiền tố liên "
                    "tục; quay về đường mask 4D."
                )
        ...
        if use_flash_attn:
            ...
            hidden_states, mid_results = self._run_decoder_sdpa(
                inputs_embeds, position_ids, token_types, n_ar,
                visual_pos_masks=visual_pos_masks,
                deepstack_visual_embeds=deepstack_visual_embeds,
                return_mid_results_layers=return_mid_results_layers)
        else:
