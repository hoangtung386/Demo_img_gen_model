...
GPU + ~10GB weight, toàn bộ chuỗi broker → worker → `queue_out` không cách
...
Delay giả lập đổi qua env `IMG_ECHO_DELAY_SECONDS` (mặc định 0.2s).
"""
...
# Trần kích thước ảnh placeholder. Model thật sinh ở >=2048² nhưng ảnh giả
# không cần to bằng — nhỏ thì upload/audit nhanh, smoke test không bị nghẽn
# ở I/O.
_MAX_SIDE = 512
...
class EchoImageGenerator(ImageGenerator):
    ...
    def __init__(self, cfg: ProcessorConfig):
        ...
        try:
            self._delay = float(
                os.environ.get(
                    "IMG_ECHO_DELAY_SECONDS", _DEFAULT_DELAY_SECONDS
                )
            )
        except ValueError:
            self._delay = _DEFAULT_DELAY_SECONDS
        logger.warning(
            "[processor] ECHO MODE — không nạp model, ảnh trả về là "
            "placeholder. "
    ...
    def generate(
        ...
        *,
        ...
        if images and match_input_size:
            ...
            side = min(_MAX_SIDE, max(self.cfg.width, self.cfg.height))
