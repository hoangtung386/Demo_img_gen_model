...
def build_object_key(
    *,
    ...
    Args:
        folder       : env/tenant root (e.g. "prd/perm"). From
        StorageConfig.folder.
        app_id       : app package name (e.g. "com.example.app"). From the
        request.
        feature      : kebab-case feature label (e.g. "gen-image"). From
        StorageConfig.feature.
        ...
        now          : optional timestamp (defaults to UTC now). Useful in
        tests.
        request_unique : optional uuid hex (defaults to uuid4). Useful in
        tests.
    ...
    filename = (
        f"os-{safe_os}_package_name-{safe_app}"
        f"_device_id-{safe_device}_{ts}_unique{unique}.{ext}"
    )

    # `folder` may legitimately contain '/' (e.g. "prd/perm"); strip
    # surrounding slashes.
    folder_clean = (folder or "").strip("/")
    return _os.path.join(
        folder_clean, safe_app, safe_feature, y, m, d, filename
    )
