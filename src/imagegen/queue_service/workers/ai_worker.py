    ...
    def _handle(self, label: str, envelope: Envelope) -> None:
        ...
        try:
            ...
            logger.opt(exception=e).error(
                f"[worker {self.worker_id}] [BAD_PAYLOAD] tier={label} "
                f"id={request_id} → DLQ"
            )
        ...
        if (
            attempt == 0
            and self.recent_requests
            and self.recent_requests.seen(msg.id)
        ):
            logger.info(
                f"[worker {self.worker_id}] [DUPLICATE] tier={label} "
                f"id={msg.id} "
