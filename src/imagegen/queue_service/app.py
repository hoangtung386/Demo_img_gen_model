...
class Application:
    ...
    Bootstrap order:
      ...
      2. init audit subsystem (storage manager + rmq error logger + cleanup
      thread)
    ...
    def run(self) -> None:
        ...
        pipeline = GenImagePipeline(
            self.settings, processor, storage, audit_storage
        )
    ...
    def _init_audit(self) -> None:
        # Initialize the rmq error logger early so messaging connection
        # failures
    ...
    def _declare_topology(self) -> None:
        try:
            ...
            logger.error(
                f"[topology] initial declare failed (will retry inside "
                f"consumers): {e}"
            )
            log_rmq_error(
    ...
    def _start_consumers(
        ...
        for tier_name, in_queue in tier_to_queue.items():
            ...
            if not queue_name:
                logger.warning(
                    f"no rabbitmq queue configured for tier {tier_name}; "
                    f"skipping"
                )
                continue
            handler = self._make_envelope_handler(
                in_queue, tier_name, publisher
            )
            consumer = RabbitMQConsumer(
                rmq, queue_name, handler, label=tier_name
            )
    ...
    def _make_envelope_handler(
        ...
        Bad payload → reply with proper status code + DLQ.
        """
