...
class StorageManager:
    ...
    Each call writes its own files (thread-safe).
    """
    ...
    def save_output_success(
        ...
        *,
        ...
        output_meta = self._copy_output_images(
            output_image_paths, record.request_dir
        )
    ...
    def _move_to_outcome(
        self, record: RequestRecord, new_outcome: OutcomeKind
    ) -> None:
        """If save_input was called with a guess,

        move the folder to the actual outcome.
        """
        ...
        try:
            ...
            with suppress(OSError):
                if old_request_root.exists() and not any(
                    old_request_root.iterdir()
                ):
    ...
    def _write_error_log(
        ...
        *,
        ...
        images = msg.get("images") or (
            [msg["image"]] if msg.get("image") else []
        )
        lines = [
            "=" * 60,
            f"  ERROR LOG - {kind}",
    ...
    def _write_traceback(
        record: RequestRecord, exception: BaseException
    ) -> None:
        try:
            tb = "".join(
