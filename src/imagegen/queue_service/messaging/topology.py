...
Declares for each consumer queue:
  • The main queue with x-dead-letter-exchange → retry exchange (routed by
  queue name)
...
Topology overview (per inbound queue Q):
    ...
    main exchange ──── routing_key=Q ────▶ Q
        ▲                                  │ on nack/expire
        │                                  ▼
        │                         retry exchange ── routing_key=Q ─▶ Q.retry
        (TTL)
        │                                                             │
...
def setup_topology(channel, cfg: RabbitMQConfig) -> None:
    ...
    channel.exchange_declare(
        exchange=main, exchange_type="direct", durable=True
    )
    channel.exchange_declare(
        exchange=retry_x, exchange_type="direct", durable=True
    )
    channel.exchange_declare(
        exchange=dlq_x, exchange_type="direct", durable=True
    )
...
def _declare_minimal(channel, cfg: RabbitMQConfig) -> None:
    ...
    không có DLX. Idempotent — restart service không lỗi.
      ...
      • for each Q in queues_in.values(): queue_declare(Q, durable=True, no
      args)
    ...
    log_topology_declared(
        exchanges=[main], queues=list(cfg.queues_in.values())
    )
    logger.info(
        f"[topology] declare_minimal — exchange={main} queues={queues} (no "
        f"DLX, no retry, no DLQ)"
    )


def _verify_passive(channel, cfg: RabbitMQConfig) -> None:
    """Verify queues exist without modifying broker state.
