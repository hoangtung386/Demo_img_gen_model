...
Pattern:
  • Consumer thread owns the pika connection + channel (pika is NOT
  thread-safe).
...
class RabbitMQConsumer:
    ...
    def run_forever(self) -> None:
        ...
        while not is_shutting_down():
            ...
            try:
                ...
                # Idempotent: ensures topology even if a fresh broker is
                # started.
                ...
                logger.info(
                    ...
                    f"prefetch={prefetch} "
                    f"(sla={self.cfg.target_sla_seconds}s, "
    ...
    def _make_on_message(self, conn: pika.BlockingConnection, channel):
        """Closure capturing this connection/channel for ack dispatch."""
        ...
        def on_message(_ch, method, properties, body) -> None:
            ...
            try:
                ...
                logger.opt(exception=e).error(
                    f"[consumer] [HANDLER_ERROR] tier={self.label} "
                    f"id={request_id} → DLQ"
                )
                envelope.dead_letter(reason=f"handler exception: {e}")
            logger.debug(
                f"[consumer] dispatched id={request_id} in "
    ...
    def _build_envelope(
        ...
        def _ack() -> None:
            conn.add_callback_threadsafe(
                lambda: _safe_ack(channel, delivery_tag)
            )
        ...
        def _publish(
            exchange: str, rkey: str, body: bytes, hdrs: dict
        ) -> None:
            conn.add_callback_threadsafe(
        ...
        return Envelope(
            ...
            # Envelope.retry()/dead_letter() phải fallback (requeue / plain
            # ack).
            _passive_mode=(
                self.cfg.topology_mode in ("passive", "declare_minimal")
            ),
        )
