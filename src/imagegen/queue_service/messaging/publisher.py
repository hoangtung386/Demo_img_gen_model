...
class RabbitMQPublisher:
    ...
    def _setup_channel(self, conn: pika.BlockingConnection):
        ...
        # declare_minimal & declare cùng path (cùng dùng exchange direct +
        # bind);
        ...
        for attempt in range(_PUBLISH_RETRIES):
            try:
                ...
                return (
                    ...
                    conn,
                    channel,
                )  # treated as handled so we don't requeue
            except Exception as e:
                logger.warning(
                ...
                channel = self._setup_channel(conn)
        log_publish_fail(
            request_id=request_id, status_code=status_code, exception=None
        )
    ...
    def _drain_on_shutdown(self, params) -> None:
        if self.publish_queue.empty():
            return
        logger.info(
            f"[publisher] draining {self.publish_queue.qsize()} pending "
            f"messages"
        )
        conn: pika.BlockingConnection | None = None
        try:
