...
Mỗi task có một "system prompt" riêng viết sẵn bằng tiếng Anh, theo cùng một
khung ba phần:

1. Hành động chính (đổi cái gì)
2. Ràng buộc bảo toàn (tuyệt đối không đổi cái gì) — phần quyết định chất
   lượng
3. Yêu cầu về độ chân thực (ánh sáng, bóng đổ, phối cảnh)

KHÔNG CÒN NEGATIVE PROMPT. HiDream-O1 không nhận nó: nhánh uncond của CFG
dùng prompt " " cố định (xem ``hidream/vendor/pipeline.py``). Sáu hằng
``*_NEGATIVE`` cũ đã bị xoá thay vì để lại một tham số không có tác dụng.

CÒN PHẢI LÀM: các prompt dưới đây được soạn cho text encoder Qwen2.5-VL của
model cũ. HiDream-O1 ăn prompt mô tả dài, tự nhiên và có sẵn Reasoning-Driven
Prompt Agent riêng. Chúng vẫn chạy nhưng chưa được A/B lại trên model mới —
xem ``docs/MODEL_HIDREAM_O1.md`` §5.
"""
...
# sẵn vài ví dụ để có chỗ bắt đầu.
