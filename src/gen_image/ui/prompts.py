...
encoder bám sát chỉ dẫn tiếng Anh tốt hơn tiếng Việt, và prompt được viết
theo cùng một khung ba phần:
...
⚠️ TRẠNG THÁI: những prompt dưới đây được viết và tinh chỉnh cho
Qwen-Image-Edit-2509 Lightning, backend TRƯỚC. Chúng đã được chuyển sang
nguyên văn và vẫn là chỉ dẫn tiếng Anh hợp lệ, nhưng CHƯA được đánh giá lại
trên FLUX.2-klein. Hai model khác nhau ở những chỗ ảnh hưởng trực tiếp tới
prompt:

  - FLUX.2 bám chỉ dẫn tốt hơn, nên phần liệt kê ràng buộc dài dòng ở đây có
    thể thừa — và với model 4 bước, prompt quá dài từng là nguyên nhân làm
    kết quả lạc (xem ghi chú ở HOME_PROMPT_TEMPLATE).
  - FLUX.2 hỗ trợ prompt dạng JSON có cấu trúc, một cách diễn đạt ràng buộc
    gọn và chính xác hơn văn xuôi.

Việc tinh chỉnh lại cần đối chứng bằng ảnh thật với người đánh giá, không
phải bằng suy luận — xem docs/REFACTOR_FLUX2_KLEIN_4B.md §2.4.

``negative prompt``: bản klein **distilled** nhúng guidance thẳng vào model
thay vì chạy CFG hai nhánh, nên diffusers tắt hẳn nhánh negative
(``do_classifier_free_guidance`` luôn False). Mọi hằng số ``*_NEGATIVE``
dưới đây vì thế KHÔNG có tác dụng ở cấu hình mặc định; chúng được giữ lại
cho bản ``klein-base`` (không distilled, guidance_scale > 1.0). Xem
...
def build_faceswap_prompt(scope_label: str) -> str:
    ...
    scope = FACE_SWAP_SCOPES.get(scope_label, next(iter(FACE_SWAP_SCOPES.values())))
