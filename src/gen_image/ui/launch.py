"""Khởi động service Gradio: nạp model rồi mở demo kèm link công khai.

Tách khỏi ``app.py`` vì đây là mối quan tâm về *tiến trình* (nạp model, tải
binary tunnel, bind cổng, thử lại) chứ không phải về *giao diện*.
``app.build_ui()`` dựng được mà không cần model — tách ra thì ranh giới đó
còn nhìn thấy được, và ``app.py`` không phải kéo theo torch lúc import.
"""
...
from ..inference import DEFAULT_OUTPUT_AREA, configure_embed_cache
from ..logging_setup import configure_logging
from ..models.loader import load_pipeline
from ..tuning import apply_gpu_tuning, maybe_compile, warmup
...
def main() -> None:
    ...
    configure_embed_cache(settings.embed_cache_size)
...
def _ensure_frpc_binary() -> None:
    ...
    for attempt in range(1, FRPC_ATTEMPTS + 1):
        logger.info(
            "Tải binary tunnel frpc (lần %d/%d): %s", attempt, FRPC_ATTEMPTS, url
        )
        try:
