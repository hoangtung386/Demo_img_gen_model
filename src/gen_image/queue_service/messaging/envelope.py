...
worker is responsible for calling exactly one of: ack(), retry(), dead_letter().
...
class Envelope:
    ...
    # KHÔNG ai gọi ở cấu hình hiện tại: AIWorker._route_failure hard-code
    # DLQ cho mọi failure. Giữ lại vì topology retry vẫn được dựng thật và
    # đây là nhánh sẽ dùng ngay khi bật. Xem docs/QUEUE_SERVICE.md mục
    # "Retry: đi dây đầy đủ nhưng ĐANG TẮT".
    def retry(self, reason: Reason = "") -> None:
        """
        Transient failure. Normally: nack without requeue → message goes to the
        retry queue via the queue's x-dead-letter-exchange, comes back after TTL.
    ...
    def dead_letter(self, reason: Reason = "") -> None:
        ...
        self._publish(self._dlq_exchange, self.routing_key, payload, self.headers)
        self._ack()
