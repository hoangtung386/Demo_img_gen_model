...
class GenImagePipeline(Pipeline):
    """
    Download 0..N ảnh tham chiếu → chạy FLUX.2-klein (edit nếu có ảnh,
    t2i nếu không) → upload kết quả → reply URL.
    ...
    def execute(
        ...
        *,
        ...
        try:
            # 1. Download ảnh tham chiếu (nếu có). Text-to-image → bỏ qua bước này.
            ...
            try:
                ...
                logger.exception(f"[pipeline] inference error id={msg.id}: {e}")
                return self._error_with_audit(
                    out,
                    msg,
            ...
            if result_image is None:
                ...
                return self._fail_with_audit(
                    out, msg, StatusCode.AI_NO_RESULT, record, started_at, worker_id
                )

            # 4. Save processed image then save_output_success (BEFORE upload).
            result_image.save(paths.predict)
            out.set_status(StatusCode.OK)
            out.result = self._build_result(
                msg, url="", info=info, image_path=paths.predict
            )
            ...
            # 5. Upload + update_output_url — bỏ qua khi provider="openai": ảnh
            # trả về dạng b64_json trong result (đã build ở bước 4, không encode
            # lại), client không cần URL public, không tốn GCS egress/storage
            # cho ảnh vốn đã nằm trong response.
            ...
            out.result = self._build_result(
                msg, url=url, info=info, image_path=paths.predict
            )
    ...
    def _classify_input_download_error(e: Exception) -> StatusCode:
        ...
        if isinstance(e, DownloadEmpty):
            return StatusCode.INPUT_INVALID_FORMAT  # 0-byte body = invalid input
    ...
    def _load_inputs(self, paths: list[str]) -> list[Image.Image]:
        max_dim = self.settings.processor.max_input_dimension
        images: list[Image.Image] = []
        for p in paths:
            with Image.open(p) as im:
                images.append(self._resize_if_oversized(im.convert("RGB"), max_dim))
        return images

    @staticmethod
    def _resize_if_oversized(im: Image.Image, max_dimension: int) -> Image.Image:
        """Thu ảnh về trong khung `max_dimension`x`max_dimension`, giữ tỉ lệ.

        Chỉ THU, không PHÓNG (ảnh nhỏ hơn giữ nguyên) — Flux2KleinPipeline
        đã tự thu ảnh tham chiếu xuống <=1024*1024 trước khi VAE-encode nên
        độ phân giải input không ảnh hưởng VRAM/tốc độ GPU.
        Resize ở đây chỉ để chặn ảnh cực lớn (chụp gốc 12-48MP) tốn RAM/băng
        thông lúc decode PIL, trước khi từng đây bytes đi xa hơn trong
        pipeline — không còn reject cứng như INPUT_DIMENSION_TOO_LARGE cũ.
        """
        if im.width <= max_dimension and im.height <= max_dimension:
            return im.copy()
    ...
    @staticmethod
    def _build_result(
        msg: InboundMessage, *, url: str, info: str, image_path: str
    ) -> dict:
        """
        Format cũ {"url", "info"} luôn có mặt (mặc định, BE/App hiện tại đọc
        field này) — `provider` chỉ THÊM một khối lồng bên cạnh, không thay
        thế, để không phá consumer đang đọc result.url/result.info trực tiếp.

        provider == "openai" → thêm result.openai theo cấu trúc OpenAI Images
        API (b64_json, không phải url — ảnh vẫn được upload GCS song song ở
        trên cho audit/storage, chỉ response thêm base64 riêng cho luồng
        này). Provider khác (vd "google") sẽ thêm nhánh tương tự sau này.
        """
        result: dict = {"url": url, "info": info}
        if msg.provider == "openai":
            result["openai"] = GenImagePipeline._build_openai_payload(image_path)
        return result

    @staticmethod
    def _build_openai_payload(image_path: str) -> dict:
    ...
    def _error_with_audit(
        ...
        return self._reply(
            out, msg, code, record, started_at, worker_id, fmt, exception=exception
        )
