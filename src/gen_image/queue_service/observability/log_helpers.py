...
def configure_helpers(
    *, payload_max_bytes: int = 500, strip_url_query: bool = True
    ...
    """Set helper-wide options. Called once at startup from ObservabilityConfig."""
...
def preview_body(body: bytes | str | None) -> str:
    """Truncate body to log_payload_max_bytes. Returns '' if disabled or empty."""
...
def _ms(elapsed_ms: float) -> str:
    """Mili-giây làm tròn, dùng trong log — tách ra để dòng gọi không quá dài."""
    return f"{elapsed_ms:.0f}"


def _n_of(current: int, total: int) -> str:
    """Dạng 'i/n' cho số lần thử."""
...
def log_inbound(
    *, tier: str, request_id: str, body: bytes, queue: str | None = None
    ...
    fields = _kv({"tier": tier, "id": request_id, "queue": queue, "size": f"{size}B"})
    preview = preview_body(body)
    suffix = f" preview={preview}" if preview else ""
    logger.info(f"[consumer] [INBOUND] {fields}{suffix}")


def log_validate_fail(*, tier: str, request_id: str, missing_field: str) -> None:
    logger.warning(
        f"[consumer] [VALIDATE_FAIL] "
        f"{_kv({'tier': tier, 'id': request_id, 'missing_field': missing_field})}"
    )
...
def log_publish_ok(*, request_id: str, status_code: int) -> None:
    logger.debug(
        f"[publisher] [PUBLISH_OK] {_kv({'id': request_id, 'status': status_code})}"
    )
...
def log_publish_fail(
    *,
    ...
    msg = f"[publisher] [PUBLISH_FAIL] {_kv({'id': request_id, 'status': status_code})}"
...
def log_process_ok(
    *, worker_id: int, request_id: str, status_code: int, elapsed_ms: float
    ...
    fields = {
        "id": request_id,
        "status": status_code,
        "elapsed_ms": _ms(elapsed_ms),
    }
    logger.info(f"[worker {worker_id}] [PROCESS_OK] {_kv(fields)}")
...
def log_process_fail(
    *,
    ...
    logger.warning(f"[worker {worker_id}] [PROCESS_FAIL] {_kv(fields)}")
    ...
    fields = _kv(
        {
            "id": request_id,
            "status": status_code,
            "elapsed_ms": f"{elapsed_ms:.0f}" if elapsed_ms is not None else None,
...
def log_retry(
    *, worker_id: int, request_id: str, attempt: int, max_attempts: int, reason: str
) -> None:
    fields = {
        "id": request_id,
        "attempt": _n_of(attempt, max_attempts),
        "reason": reason,
    }
    logger.warning(f"[worker {worker_id}] [RETRY] {_kv(fields)}")


def log_dlq(*, worker_id: int, request_id: str, reason: str) -> None:
    logger.error(
        f"[worker {worker_id}] [DLQ] {_kv({'id': request_id, 'reason': reason})}"
    )
...
def log_connection_lost(
    *, component: str, label: str, queue: str | None = None, exception: BaseException
) -> None:
    fields = _kv({"label": label, "queue": queue, "exc": type(exception).__name__})
    logger.opt(exception=exception).error(f"[{component}] [CONNECTION_LOST] {fields}")
