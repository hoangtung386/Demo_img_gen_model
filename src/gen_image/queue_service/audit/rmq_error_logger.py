...
def notify_connection_recovered(component: str, label: str, **context_kv: Any) -> None:
    """
    ...
    _write_event_file(
        ...
        context={
            **context,
            ...
            "first_failure_at": datetime.fromtimestamp(state["first_ts"]).isoformat(),
...
def _identity_key(event: str, context: dict[str, Any]) -> str:
    ...
    if event == "consumer_connection_lost":
        return f"consumer={context.get('tier') or context.get('label') or 'default'}"
    if event == "publisher_connection_error":
        return (
            f"publisher={context.get('queue_out') or context.get('label') or 'default'}"
        )
    if event == "topology_declare_failed":
        return (
            f"topology={context.get('exchange') or context.get('label') or 'default'}"
        )
    return "default"
