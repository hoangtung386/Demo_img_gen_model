...
class AuditCleaner:
    ...
    def run_forever(self) -> None:
        ...
        logger.info(
            ...
            f"interval={self.cfg.cleanup_interval_hours}h root={self.service_root}"
        )
    ...
    def _run_once(self) -> None:
        ...
        if removed:
            logger.info(
                f"[audit-cleaner] removed {removed} day-folder(s) older than {cutoff}"
            )
            ...
            for month_dir in self._numeric_subdirs(year_dir):
                for day_dir in self._numeric_subdirs(month_dir):
                    ...
                    try:
                        ...
                        logger.warning(f"[audit-cleaner] rmtree {day_dir}: {e}")
...
def start_cleanup_thread(cfg: AuditConfig) -> None:
    ...
    t = threading.Thread(target=cleaner.run_forever, name="audit-cleaner", daemon=True)
    t.start()
